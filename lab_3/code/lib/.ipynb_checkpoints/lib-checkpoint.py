...
def collate_fn(batch, pad_id, max_len):
    ...
    srcs = [torch.tensor(s[:max_len], dtype=torch.long) for s in srcs]
    tgts = [torch.tensor(t[:max_len], dtype=torch.long) for t in tgts]

    srcs = pad_sequence(srcs, batch_first=True, padding_value=pad_id)
    tgts = pad_sequence(tgts, batch_first=True, padding_value=pad_id)
    return srcs, tgts

def generate_story_end(model, tokenizer, start_text, device, max_new_tokens=200, temperature=1.0, top_k=50):
    model.eval()
    with torch.no_grad():
        src_ids = tokenizer.encode(start_text, add_special_tokens=False)
        src = torch.tensor([[tokenizer.bos_token_id] + src_ids + [tokenizer.eos_token_id]], dtype=torch.long).to(device)
        memory, src_mask = model.encode(src)

        tgt = torch.tensor([[tokenizer.bos_token_id]], dtype=torch.long).to(device)
        ...
        for _ in range(max_new_tokens):
            ...
            tgt = torch.cat([tgt, next_token.unsqueeze(-1)], dim=1)  # (batch, seq_len+1)
            
            if next_token.item() == tokenizer.eos_token_id:
                break

        generated = tokenizer.decode(tgt[0].tolist(), skip_special_tokens=True)
        return generated
